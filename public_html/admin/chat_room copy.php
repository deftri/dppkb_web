...
$sql_chat = "SELECT * FROM chat_sessions WHERE id = ?";
$stmt_chat = $conn->prepare($sql_chat);
$stmt_chat->bind_param("i", $session_id);
$stmt_chat->execute();
$chat_session = $stmt_chat->get_result()->fetch_assoc();

if (!$chat_session) {
    echo "Sesi chat tidak ditemukan.";
    exit();
}
...
if ($role === 'klien') {
    if ($refer_status == 0) {
        ...
        $display_name = "PSIKOLOG: " . strtoupper(htmlspecialchars($psychologist_info['psychologist_name']));
    }
}
...
// For konselor or psikolog: show client name and wilayah
    ...
    $display_name = strtoupper(htmlspecialchars($client_info['client_name'])) . " - " . strtoupper(htmlspecialchars($client_info['wilayah']));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['exit_chat'])) {
    ...
    $redirect_path = ($role == 'klien') ? '../klien/dashboard-klien.php' :
